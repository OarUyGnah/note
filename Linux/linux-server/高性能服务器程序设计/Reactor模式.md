...
![image-20220712154551249](../image/image-20220712154551249.png)
...
bool create_server_listener(const char* ip, short port)
{
    ...
    int on = 1;
    //无需等待，可强制使用被处于TIME_WAIT状态的连接占用的socket地址和端口
...
{   
    while (!g_bStop)
    {
        ...
        //从pthread_create开始，list空则一直等，直到main函数push_back后，pthread_cond_signal(&g_cond);
...
{  
    ...
    ::pthread_mutex_init(&g_clientmutex, NULL);
     //专门用于接收新clientfd，对应accept_thread_func函数
    ::pthread_create(&g_acceptthreadid, NULL, accept_thread_func, NULL);
    //启动工作线程，对应worker_thread_func函数
    ...
    {       
        ...
        for (int i = 0; i < m; ++i)
        {
            ...
            if (ev[i].data.fd == g_listenfd)
                //触发accept_thread_func的pthread_cond_wait
...
注意注释部分，在Unix平台上设置这个选项意味着，任意进程可以复用该地址；而在windows，不要阻止其他进程复用该地址。也就是在**在Unix平台上，如果不设置这个选项，任意进程在一定时间内，不能bind该地址；在windows平台上，在一定时间内，其他进程不能bind该地址，而本进程却可以再次bind该地址**。
...
![image-20220712181008940](../image/image-20220712181008940.png)

## c++11版本

使用CMake编译，为了支持编译必须加上这-std=c++11：
