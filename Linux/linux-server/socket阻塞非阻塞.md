...
![image-20220706142343331](image/image-20220706142343331.png)
...
![image-20220706143731610](image/image-20220706143731610.png)

当每次 **blocking_client** 给 **blocking_server** 发数据以后，**blocking_server** 会应答 **blocking_server**，在每次应答的数据包中会带上自己的当前可用 TCP 窗口大小（看上文中结果从 **127.0.0.1.3000 > 127.0.0.1.40846**方向的数据包的 **win** 字段大小变化），由于 TCP 流量控制和拥塞控制机制的存在，**blocking_server** 端的 TCP 窗口大小短期内会慢慢增加，后面随着接收缓冲区中数据积压越来越多， TCP 窗口会慢慢变小，最终变为 0。
...
![image-20220706143924197](image/image-20220706143924197.png)
...
![image-20220706144722783](image/image-20220706144722783.png)