...
#define PACK(size, alloc) ((size) | (alloc)) // alloc == 1 已分配
 ...
 * 从地址p处的头部或脚部得到块大小和分配位
 */
#define GET_SIZE(p) (GET(p) & ~0x7)
#define GET_ALLOC(p) (GET(p) & 0x1)

/* Given block ptr bp, compute address of its header and footer
 * 根据块指针(payload)的到当前块的头部和脚部指针
 */
...
#define PREV_BLKP(bp) ((char*)(bp)-GET_SIZE(((char*)(bp)-DSIZE))) // -DSIZE到达上一块的脚部，并计算大小
